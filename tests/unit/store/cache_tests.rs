...
fn test_cache_creation() {
    ...
    let cache = CachedData::new(data.clone(), 1234567890, 42);

    assert_eq!(cache.mtime, 1234567890);
    assert_eq!(cache.inode, 42);
...
fn test_cache_is_valid_same_mtime() {
    ...
    let cache = CachedData::new(data, 1000, 10);

    assert!(cache.is_valid(1000, 10));
}
...
fn test_cache_is_invalid_different_mtime() {
    ...
    assert!(!cache.is_valid(2000, 10));
    assert!(!cache.is_valid(999, 10));
    assert!(!cache.is_valid(1000, 20));
}

#[test]
fn test_cache_clone() {
    let data = Arc::new(json!({"test": 123}));
    let cache1 = CachedData::new(data.clone(), 5000, 5);
    let cache2 = cache1.clone();

    assert_eq!(cache1.mtime, cache2.mtime);
    assert_eq!(cache1.inode, cache2.inode);
...
fn test_cache_arc_sharing() {
    ...
    let cache1 = CachedData::new(data.clone(), 100, 1);
    let cache2 = CachedData::new(data.clone(), 100, 1);
...
fn test_cache_with_complex_data() {
    ...
    let cache = CachedData::new(data, 999, 9);
    assert!(cache.is_valid(999, 9));
