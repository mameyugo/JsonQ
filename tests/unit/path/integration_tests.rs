...
fn test_build_complex_structure() {
    ...
    assert_eq!(read_path(&data, "company.employees.0.name"), Some(&json!("Alice")));
    assert_eq!(read_path(&data, "company.employees.1.role"), Some(&json!("Manager")));
}
...
fn test_partial_path_removal() {
    ...
    assert_eq!(read_path(&data, "user.profile.email"), Some(&json!("alice@example.com")));
    assert_eq!(read_path(&data, "user.settings.theme"), Some(&json!("dark")));
...
fn test_realistic_user_data_scenario() {
    ...
    assert_eq!(read_path(&data, "users.0.roles"), Some(&json!(["admin", "editor"])));
    assert_eq!(read_path(&data, "users.0.profile.bio"), Some(&json!("Software engineer")));
    
    // Update email
    write_path(&mut data, "users.0.email", json!("alice.new@example.com"));
    assert_eq!(read_path(&data, "users.0.email"), Some(&json!("alice.new@example.com")));
    
    // Remove bio
    remove_path(&mut data, "users.0.profile.bio");
    assert_eq!(read_path(&data, "users.0.profile.bio"), None);
    assert_eq!(read_path(&data, "users.0.profile.location"), Some(&json!("NYC")));
}
