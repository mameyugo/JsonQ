...
LATEST_TAG=$(curl -fsSL "https://api.github.com/repos/mameyugo/JsonQ/releases/latest" | grep '"tag_name"' | sed 's/.*"v\(.*\)".*/\1/' 2>/dev/null || echo "")
...
if curl -fsSL "$DEB_URL" -o "/tmp/${DEB_FILE}"; then
    ...
    echo -e "${RED}✗ Download failed. Available packages at:${NC}"
    echo "  ${GITHUB_RELEASES}/latest"
    exit 1
fi
