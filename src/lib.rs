...
#[php(class, name = "JsonQ\\Store")]
pub struct JsonStore { inner: Option<StoreInner> }

#[php(impl)]
impl JsonStore {
    #[php_method] pub fn __construct(path: String) -> Self { Self { inner: Some(StoreInner::new(path)) } }
    ...
    #[php_method] pub fn beginTransaction(&self) -> PhpResult<bool> {
        ...
        let data = i.read().map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))?;
    ...
    #[php_method] pub fn inTransaction(&self) -> bool { self.inner.as_ref().map(|i| *i.in_transaction.read().unwrap()).unwrap_or(false) }
    ...
    #[php_method] pub fn getAll(&self) -> Zval { self.inner.as_ref().and_then(|i| i.read().map(|cd| value_to_zval(&cd)).ok()).unwrap_or_else(Zval::new) }
    ...
    #[php_method] pub fn get(&self, path: String) -> Zval { let i = match &self.inner { Some(i) => i, None => return Zval::new() }; i.read().map(|cd| rp(&cd, &path).map(|v| value_to_zval(v)).unwrap_or_else(Zval::new)).unwrap_or_else(|_| Zval::new()) }
    #[php_method] pub fn has(&self, path: String) -> bool { self.inner.as_ref().and_then(|i| i.read().map(|cd| rp(&cd, &path).is_some()).ok()).unwrap_or(false) }
    #[php_method] pub fn count(&self, path: String) -> i64 { self.inner.as_ref().and_then(|i| i.read().map(|cd| match rp(&cd, &path) { Some(Value::Array(a)) => a.len() as i64, Some(Value::Object(o)) => o.len() as i64, _ => -1 }).ok()).unwrap_or(-1) }
    #[php_method] pub fn keys(&self, path: String) -> Vec<String> { self.inner.as_ref().and_then(|i| i.read().map(|cd| match if path.is_empty(){Some(cd.as_ref())}else{rp(&cd,&path)} { Some(Value::Object(o)) => o.keys().cloned().collect(), _ => vec![] }).ok()).unwrap_or_default() }

    #[php_method] pub fn set(&self, path: String, value: &Zval) -> bool { let i = match &self.inner { Some(i) => i, None => return false }; let v = zval_to_value(value); i.mutate(|d| sap(d, &path, v)).is_ok() }
    #[php_method] pub fn remove(&self, path: String) -> bool { let i = match &self.inner { Some(i) => i, None => return false }; i.mutate(|d| { rap(d, &path); }).is_ok() }
    #[php_method] pub fn push(&self, path: String, value: &Zval) -> bool { let i = match &self.inner { Some(i) => i, None => return false }; let v = zval_to_value(value); i.mutate(|d| { match if path.is_empty(){Some(d)}else{rpm(d, &path)} { Some(Value::Array(a)) => { a.push(v); } _ => {} } }).is_ok() }
    #[php_method] pub fn merge(&self, path: String, value: &Zval) -> bool { let i = match &self.inner { Some(i) => i, None => return false }; let nv = zval_to_value(value); i.mutate(|d| { if let Some(e) = if path.is_empty(){Some(&mut *d)}else{rpm(d,&path)} { merge_v(e, &nv); } else { sap(d, &path, nv); } }).is_ok() }
    #[php_method] pub fn increment(&self, path: String, amount: Option<f64>) -> bool { let amt = amount.unwrap_or(1.0); let i = match &self.inner { Some(i) => i, None => return false }; i.mutate(|d| { if let Some(v) = rpm(d, &path) { if let Some(n) = v.as_f64() { *v = json!(n + amt); } } }).is_ok() }
    #[php_method] pub fn decrement(&self, path: String, amount: Option<f64>) -> bool { self.increment(path, Some(-(amount.unwrap_or(1.0)))) }
    ...
    #[php_method] pub fn findOne(&self, collection: String, conditions: &Zval) -> Zval { let i = match &self.inner { Some(i) => i, None => return Zval::new() }; let c = zval_to_value(conditions); i.read().map(|cd| match rp(&cd, &collection) { Some(Value::Array(a)) => a.iter().find(|item| mat(item, &c)).map(|f| value_to_zval(f)).unwrap_or_else(Zval::new), _ => Zval::new() }).unwrap_or_else(|_| Zval::new()) }
    #[php_method] pub fn executeQuery(&self, collection: String, query_spec: &Zval) -> Zval { let i = match &self.inner { Some(i) => i, None => return Zval::new() }; let q = zval_to_value(query_spec); i.read().map(|cd| match rp(&cd, &collection) { Some(Value::Array(a)) => value_to_zval(&Value::Array(exec_fluent(a, &q))), _ => value_to_zval(&Value::Array(vec![])) }).unwrap_or_else(|_| Zval::new()) }

    #[php_method] pub fn aggregate(&self, collection: String, field: String, operation: String) -> Zval { let i = match &self.inner { Some(i) => i, None => return Zval::new() }; i.read().map(|cd| match rp(&cd, &collection) { Some(Value::Array(a)) => value_to_zval(&agg(a, &field, &operation)), _ => Zval::new() }).unwrap_or_else(|_| Zval::new()) }
    #[php_method] pub fn groupBy(&self, collection: String, field: String) -> Zval { let i = match &self.inner { Some(i) => i, None => return Zval::new() }; i.read().map(|cd| match rp(&cd, &collection) { Some(Value::Array(a)) => value_to_zval(&grp(a, &field)), _ => Zval::new() }).unwrap_or_else(|_| Zval::new()) }
    #[php_method] pub fn pluck(&self, collection: String, fields: Vec<String>) -> Zval { let i = match &self.inner { Some(i) => i, None => return Zval::new() }; let fr: Vec<&str> = fields.iter().map(|s| s.as_str()).collect(); i.read().map(|cd| match rp(&cd, &collection) { Some(Value::Array(a)) => value_to_zval(&Value::Array(plk(a, &fr))), _ => Zval::new() }).unwrap_or_else(|_| Zval::new()) }

    #[php_method] pub fn validate(&self, path: String, schema: &Zval) -> Zval { let i = match &self.inner { Some(i) => i, None => return Zval::new() }; let sv = zval_to_value(schema); i.read().map(|cd| { let t = if path.is_empty(){cd.as_ref()}else{match rp(&cd,&path){Some(v)=>v,None=>return Zval::new()}}; let e = vld(t, &sv, &path); value_to_zval(&json!({"valid":e.is_empty(),"error_count":e.len(),"errors":e})) }).unwrap_or_else(|_| Zval::new()) }
    ...
    #[php_method] pub fn createIndex(&self, collection: String, field: String) -> bool { self.inner.as_ref().map(|i| i.build_index(&collection, &field).is_ok()).unwrap_or(false) }
    #[php_method] pub fn createCompoundIndex(&self, collection: String, fields: Vec<String>) -> bool { self.inner.as_ref().map(|i| i.build_compound(&collection, &fields).is_ok()).unwrap_or(false) }
    #[php_method] pub fn indexLookup(&self, collection: String, field: String, value: &Zval) -> Zval {
        ...
        if let Some(pos) = i.idx_lookup(&collection, &field, &v) {
            i.read().map(|cd| if let Some(Value::Array(a)) = rp(&cd, &collection) { value_to_zval(&Value::Array(pos.iter().filter_map(|&j| a.get(j).cloned()).collect())) } else { Zval::new() }).unwrap_or_else(|_| Zval::new())
        } else { Zval::new() }
    }
    #[php_method] pub fn listIndexes(&self) -> Zval {
        let i = match &self.inner { Some(i) => i, None => return Zval::new() }; let idx = i.indexes.read().unwrap();
        let mut r = Vec::new();
        for (c, s) in idx.iter() {
            for (f, im) in &s.single { r.push(json!({"collection":c,"type":"single","field":f,"unique_values":im.len(),"total_entries":im.values().map(|v|v.len()).sum::<usize>()})); }
            for (f, im) in &s.compound { r.push(json!({"collection":c,"type":"compound","fields":f,"unique_values":im.len(),"total_entries":im.values().map(|v|v.len()).sum::<usize>()})); }
        }
    ...
    #[php_method] pub fn stats(&self) -> Zval {
    ...
    #[php_method] pub fn backup(&self, backup_path: Option<String>) -> PhpResult<String> {
        let i = self.inner.as_ref().ok_or("Not init")?;
        ...
        fs::copy(&i.path, &t).map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))?;
        Ok(t)
    }
    #[php_method] pub fn restore(&self, backup_path: String) -> PhpResult<bool> {
        let i = self.inner.as_ref().ok_or("Not init")?;
        fs::copy(&backup_path, &i.path).map_err(|e| ext_php_rs::exception::PhpException::default(e.to_string()))?;
        *i.cache.write().unwrap() = None; i.indexes.write().unwrap().clear();
        Ok(true)
    }
}

#[php(function)] pub fn jsonq_version() -> String { env!("CARGO_PKG_VERSION").to_string() }
#[php(module)] pub fn get_module(module: ModuleBuilder) -> ModuleBuilder { module }
