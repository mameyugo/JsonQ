...
impl JsonStore {
    ...
    pub fn setOption(&self, key: String, value: &Zval) -> bool {
    ...
    pub fn getOption(&self, key: String) -> Zval {
    ...
    pub fn beginTransaction(&self) -> PhpResult<bool> {
    ...
    pub fn inTransaction(&self) -> bool { self.inner.as_ref().map(|i| *i.in_transaction.read().unwrap()).unwrap_or(false) }

    pub fn setMany(&self, pairs: &Zval) -> PhpResult<i64> {
        let i = self.inner.as_ref().ok_or("Not init")?;
    ...
    pub fn removeMany(&self, paths: Vec<String>) -> PhpResult<i64> {
        let i = self.inner.as_ref().ok_or("Not init")?;
        let cd = i.read().map_err(|e: String| ext_php_rs::exception::PhpException::from(e.to_string()))?;
        let mut data = (*cd).clone();
        let mut count = 0i64;
    ...
    pub fn toJson(&self, pretty: Option<bool>) -> PhpResult<String> {
        let i = self.inner.as_ref().ok_or("Not init")?;
        let cd = i.read().map_err(|e: String| ext_php_rs::exception::PhpException::from(e.to_string()))?;
    ...
    pub fn fromJson(&self, json_str: String) -> PhpResult<bool> {
        let i = self.inner.as_ref().ok_or("Not init")?;
    ...
    pub fn getAll(&self) -> Zval { self.inner.as_ref().and_then(|i| i.read().map(|cd| value_to_zval(&cd)).ok()).unwrap_or_else(Zval::new) }
    ...
    pub fn get(&self, path: String) -> Zval { let i = match &self.inner { Some(i) => i, None => return Zval::new() }; i.read().map(|cd| rp(&cd, &path).map(|v| value_to_zval(v)).unwrap_or_else(Zval::new)).unwrap_or_else(|_| Zval::new()) }
    pub fn has(&self, path: String) -> bool { self.inner.as_ref().and_then(|i| i.read().map(|cd| rp(&cd, &path).is_some()).ok()).unwrap_or(false) }
    pub fn count(&self, path: String) -> i64 { self.inner.as_ref().and_then(|i| i.read().map(|cd| match rp(&cd, &path) { Some(Value::Array(a)) => a.len() as i64, Some(Value::Object(o)) => o.len() as i64, _ => -1 }).ok()).unwrap_or(-1) }
    pub fn keys(&self, path: String) -> Vec<String> { self.inner.as_ref().and_then(|i| i.read().map(|cd| match if path.is_empty(){Some(cd.as_ref())}else{rp(&cd,&path)} { Some(Value::Object(o)) => o.keys().cloned().collect(), _ => vec![] }).ok()).unwrap_or_default() }

    pub fn set(&self, path: String, value: &Zval) -> bool { let i = match &self.inner { Some(i) => i, None => return false }; let v = zval_to_value(value); i.mutate(|d| sap(d, &path, v)).is_ok() }
    pub fn remove(&self, path: String) -> bool { let i = match &self.inner { Some(i) => i, None => return false }; i.mutate(|d| { rap(d, &path); }).is_ok() }
    pub fn push(&self, path: String, value: &Zval) -> bool { let i = match &self.inner { Some(i) => i, None => return false }; let v = zval_to_value(value); i.mutate(|d| { match if path.is_empty(){Some(d)}else{rpm(d, &path)} { Some(Value::Array(a)) => { a.push(v); } _ => {} } }).is_ok() }
    pub fn merge(&self, path: String, value: &Zval) -> bool { let i = match &self.inner { Some(i) => i, None => return false }; let nv = zval_to_value(value); i.mutate(|d| { if let Some(e) = if path.is_empty(){Some(&mut *d)}else{rpm(d,&path)} { merge_v(e, &nv); } else { sap(d, &path, nv); } }).is_ok() }
    pub fn increment(&self, path: String, amount: Option<f64>) -> bool { let amt = amount.unwrap_or(1.0); let i = match &self.inner { Some(i) => i, None => return false }; i.mutate(|d| { if let Some(v) = rpm(d, &path) { if let Some(n) = v.as_f64() { *v = json!(n + amt); } } }).is_ok() }
    ...
    pub fn aggregate(&self, collection: String, field: String, operation: String) -> Zval { let i = match &self.inner { Some(i) => i, None => return Zval::new() }; i.read().map(|cd| match rp(&cd, &collection) { Some(Value::Array(a)) => value_to_zval(&agg(a, &field, &operation)), _ => Zval::new() }).unwrap_or_else(|_| Zval::new()) }
    pub fn groupBy(&self, collection: String, field: String) -> Zval { let i = match &self.inner { Some(i) => i, None => return Zval::new() }; i.read().map(|cd| match rp(&cd, &collection) { Some(Value::Array(a)) => value_to_zval(&grp(a, &field)), _ => Zval::new() }).unwrap_or_else(|_| Zval::new()) }
    pub fn pluck(&self, collection: String, fields: Vec<String>) -> Zval { let i = match &self.inner { Some(i) => i, None => return Zval::new() }; let fr: Vec<&str> = fields.iter().map(|s| s.as_str()).collect(); i.read().map(|cd| match rp(&cd, &collection) { Some(Value::Array(a)) => value_to_zval(&Value::Array(plk(a, &fr))), _ => Zval::new() }).unwrap_or_else(|_| Zval::new()) }
    ...
    pub fn validateCollection(&self, path: String, item_schema: &Zval) -> Zval {
    ...
    pub fn createIndex(&self, collection: String, field: String) -> bool { self.inner.as_ref().map(|i| i.build_index(&collection, &field).is_ok()).unwrap_or(false) }
    pub fn createCompoundIndex(&self, collection: String, fields: Vec<String>) -> bool { self.inner.as_ref().map(|i| i.build_compound(&collection, &fields).is_ok()).unwrap_or(false) }
    pub fn indexLookup(&self, collection: String, field: String, value: &Zval) -> Zval {
    ...
    pub fn dropIndex(&self, collection: String) -> bool { self.inner.as_ref().map(|i| i.indexes.write().unwrap().remove(&collection).is_some()).unwrap_or(false) }
    pub fn dropAllIndexes(&self) -> i64 { self.inner.as_ref().map(|i| { let mut idx = i.indexes.write().unwrap(); let c = idx.len() as i64; idx.clear(); c }).unwrap_or(0) }
