...
### Added
- **Database Branching**: Implemented Turso-compatible database branching capabilities for development, staging, and isolated testing.
  - Create isolated database branches (`createBranch`).
  - Switch active connection between branches (`switchBranch`).
  - Deep-merge modified branch data back into the current branch (`mergeBranch`).
  - List and delete database branches (`listBranches`, `deleteBranch`).
- **GitHub Action Node.js 24 Migration**: Resolved deprecation warnings for Node.js 20 by upgrading GitHub Actions (Checkout, Upload/Download Artifact, GitHub Release) to versions running natively on Node.js 24.
